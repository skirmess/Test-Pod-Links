...
Some web servers do not answer `HEAD` requests but serve the same URL
without a complaint on a `GET` request. If the `HEAD` request returns a
response from the server that indicates an error, the link is retried with
the `get` method before it is reported as broken. A request that did not
...
never be verified and has to be excluded. Use `ignore_match` to exclude a
...
The user agent must implement the `head` method. A user agent that also
implements the `get` method is additionally used to retry links whose
`HEAD` request failed.
